...
DATA_DIR="${DATA_DIR:-data/FairFace/fairface-img-margin125-trainval}"
...
RUN_ID="${RUN_ID:-RUN_$(date -u +%Y%m%d_%H%M%S)_FAIRFACE_MARGIN125_FULL}"

MANIFEST="${MANIFEST:-manifests/${DATASET_ID}_fairface_margin125.csv}"
VALIDATION_REPORT="${VALIDATION_REPORT:-output/validation/${DATASET_ID}_fairface_margin125_validation.json}"
...
echo "[1/2] Preparing FairFace margin 1.25 manifest"
"${PYTHON_BIN}" -u scripts/prepare_fairface.py \
    --data-dir "${DATA_DIR}" \
    --train-labels "${TRAIN_LABELS}" \
    --val-labels "${VAL_LABELS}" \
    --dataset-id "${DATASET_ID}" \
    --dataset-version margin125 \
    --image-padding 1.25 \
...
WANDB_ARGS=(
    ...
    --wandb-tags fairface margin125 full-dataset "${PROVIDER}" interval-aware
)
if [[ -n "${WANDB_ENTITY:-}" ]]; then
    WANDB_ARGS+=(--wandb-entity "${WANDB_ENTITY}")
fi

echo "[2/2] Benchmarking the complete FairFace margin 1.25 dataset"
